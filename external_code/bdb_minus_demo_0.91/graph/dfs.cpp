...
void DFSClass::dosearch()
{
  ...
  if (!localStack_node_i) {
    printf("Fail to do: long * localStack_node_i = new long [nnode];\n");
    goto Label_FreeMemory_Return;
  }

  localStack_node_j = new long [nnode+2*stackProtectLen];
  if (!localStack_node_j) {
    printf("Fail to do: long * localStack_node_j = new long [nnode];\n");
    goto Label_FreeMemory_Return;
  }
  ...
  for (i=0;i<nnode;i++) {
	...
	printf("The local stack is overflow idx=%i!!!\n",stackIdx);
	goto Label_FreeMemory_Return;
      }
      if (b_disp) {
	printf("push %i %i statckIdx = %i\n",i,j, stackIdx);
      }
	...
	if (nodeColor[j]==ColorWhite && adjMatrix2d[j][i]!=0) {
	  ...
	  stackIdx++;
	  if (stackIdx>nnode+stackProtectLen) {
	    printf("The local stack is overflow idx=%i i=%i j=%i nnode=%i!!!\n",stackIdx,i,j,nnode);
	    for (tmpi=stackProtectLen;tmpi<=stackIdx;tmpi++)
	      printf("stack pos [%i] i=%i j=%i\n",tmpi,i,j);
	    goto Label_FreeMemory_Return;
	  }
	  if (b_disp) {
	    printf("push i=%i j=%i statckIdx = %i\n",i,j, stackIdx);
	  }
	...
	if (stackIdx<stackProtectLen) {
	  printf("The local stack is underflow!!!\n");
	  goto Label_FreeMemory_Return;
	}
	if (b_disp) {
	  printf("pop i=%i j=%i stackIdx = %i\n",i,j,stackIdx);
	}
